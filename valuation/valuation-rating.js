...
(() => {
  ...
  function findFactorScore(rows, factor) {
    for (let r = 0; r < rows.length; r += 1) {
      ...
      for (let c = 0; c < row.length; c += 1) {
        ...
        // 1) nearby explicit numeric score / grade text
        ...
        // 2) marked rating table (● / ○ etc.) with headers above
        for (let cc = c + 1; cc <= Math.min(row.length - 1, c + 12); cc += 1) {
  ...
  function rowHtml(item) {
    const scoreHtml = item.score
      ? `${item.score}점<br><span>${GRADE[item.score]}</span>`
      : '정보없음';
    return `<div class="myeong-rating-row">
      <div class="myeong-rating-name">${item.name.replace(/([가-힣])([A-Z])/g,'$1 $2')}</div>
      <div class="myeong-rating-definition">${item.definition}</div>
      <div class="myeong-rating-score ${item.score ? '' : 'is-missing'}">${scoreHtml}</div>
    </div>`;
  }
  ...
  function renderRatings() {
    ...
    section.innerHTML = `<h2>3. 세부 평가등급</h2>
      <div class="myeong-rating-layout">
        <div class="myeong-rating-column">${cards[0]}${cards[1]}</div>
        <div class="myeong-rating-column">${cards[2]}${cards[3]}</div>
      </div>
      <div class="myeong-rating-scale"><strong>평점 기준</strong> · 1 매우 미흡 · 2 미흡 · 3 보통 · 4 우수 · 5 매우 우수</div>
      <div class="myeong-rating-note">* 평가요인 정의는 IP담보평가 평가요약 기준을 적용하며, 점수는 업로드 Excel에서 확인 가능한 값을 표시함.</div>`;
  ...
  const scheduleRender = () => requestAnimationFrame(() => requestAnimationFrame(renderRatings));
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (reportArea) new MutationObserver(scheduleRender).observe(reportArea,{childList:true,subtree:true});
  });
})();
